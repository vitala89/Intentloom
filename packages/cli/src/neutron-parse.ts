...
export interface NeutronParsedArguments {
  readonly section: string;
  readonly nestedAction: string;
  readonly flags: ReadonlySet<string>;
  readonly values: ReadonlyMap<string, string>;
}

export function parseNeutronArguments(
  args: readonly string[],
): NeutronParsedArguments {
  const section = args[1] ?? "";
  if (!["subagent", "sync"].includes(section))
    throw new CliUsageError("neutron requires subagent or sync subcommand");

  const nestedAction = section === "subagent" ? (args[2] ?? "") : "";
  const startIndex = section === "subagent" ? 3 : 2;
  ...
  return { section, nestedAction, flags, values };
}
