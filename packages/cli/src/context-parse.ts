...
export interface ContextParsedArguments {
  readonly flags: ReadonlySet<string>;
  readonly values: ReadonlyMap<string, string>;
  readonly mappingValues: ReadonlyMap<string, readonly string[]>;
}

export function parseContextArguments(
  args: readonly string[],
): ContextParsedArguments {
  if (args[1] !== "get") {
  ...
  assertDaemonFlagsAllowed("context", daemonEndpoint, daemonTokenFile);
  if (flags.has("--cache")) {
    throw new CliUsageError("--cache is only valid with clean");
  }

  return { flags, values, mappingValues };
}
