...
- Engineering Quality Packs phases Q1–Q4 are implemented, verified, and merged
  ...
  transitive affected project resolution, and test suite are complete and merged through PR #265 (`852ddf7`). Phase Q13 CLI and JSON Surface is implemented on branch `feat/engineering-quality-q13-cli-json-surface`: its implementation plan, quality/standards/packs/checkers/graph CLI command handlers in `packages/application/src/engineering-quality/cli-commands.ts`, deterministic `--json` machine-readable output, stable exit codes (0/1), non-mutation invariant, and test suite `tests/cli-engineering-quality.test.ts` (14 tests) are complete.
