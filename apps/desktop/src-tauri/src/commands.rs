...
#[tauri::command]
pub async fn invoke_specialized_pack_activate_request(
    app: AppHandle,
    state: State<'_, DaemonRuntime>,
    request: Value,
) -> Result<Value, BridgeError> {
    ...
    run_blocking(move || {
        let method = request
            ...
            .ok_or_else(|| {
                BridgeError::new(
                    "unsupported_capability",
                    "missing specialized pack activate method",
                )
            })?;
        if !is_specialized_pack_activate_method(method) {
            return Err(BridgeError::new(
                "unsupported_capability",
...
#[tauri::command]
pub async fn list_neutron_mutation_reviews(
    app: AppHandle,
    state: State<'_, DaemonRuntime>,
    request: Value,
) -> Result<Value, BridgeError> {
    ...
    run_blocking(move || {
        let method = request
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| {
                BridgeError::new("unsupported_capability", "missing neutron review method")
            })?;
        if !is_neutron_mutation_review_list_method(method) {
            return Err(BridgeError::new(
                "unsupported_capability",
                "desktop command is not allowed for this neutron review operation",
            ));
        }
        state
            .ensure_daemon(&app, &request)
...
#[tauri::command]
pub async fn get_neutron_mutation_review(
    app: AppHandle,
    state: State<'_, DaemonRuntime>,
    request: Value,
) -> Result<Value, BridgeError> {
    ...
    run_blocking(move || {
        ...
        if !is_neutron_mutation_review_get_method(method) {
            return Err(BridgeError::new(
                "unsupported_capability",
                "desktop command is not allowed for this neutron review operation",
            ));
