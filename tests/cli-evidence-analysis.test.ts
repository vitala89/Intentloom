...
const macosNode22 =
  process.platform === "darwin" && process.versions.node.startsWith("22.");
const cliEvidenceAnalysisTestTimeout = macosNode22 ? 15_000 : 5_000;

describe("release evidence analysis CLI", () => {
  it(
    "combines local timeline evidence with an explicit provider export",
    async () => {
    ...
    cliEvidenceAnalysisTestTimeout,
  );
});
