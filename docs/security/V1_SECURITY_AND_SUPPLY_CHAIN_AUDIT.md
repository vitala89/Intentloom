...
| **Package Provenance**    | Signed npm releases and GitHub release provenance                                              | ⚠️ NOT MET for `1.0.0`; mechanism in place for later releases | `intentloom@1.0.0` was published manually on 2026-07-30, before `.github/workflows/release.yml` existed, and `npm view intentloom@1.0.0` reports no `dist.attestations`. npm does not allow a published version to be replaced, so `1.0.0` cannot gain provenance retroactively. The release workflow now on `main` publishes via npm trusted publishing, which attaches provenance automatically, once the npm-side trusted publisher and the `npm-publish` environment reviewer are configured. Reproducibility was verified instead: a local build reproduces the published tarball shasum exactly. |
