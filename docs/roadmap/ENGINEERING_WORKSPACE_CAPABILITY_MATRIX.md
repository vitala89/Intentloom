...
| Adoption plan / apply                         | implemented | adoption contracts                            | yes       | read-only plan/decisions/prepare/revalidate/approve RPCs; apply still denied on Desktop | `adopt`, `update`                        | preview + decisions + prepared-plan + explicit approval | partial                 | partial                               | `integration-pending` | Pre-Apply security review accepted bounded Apply next (`DESKTOP_ADOPTION_PRE_APPLY_SECURITY_REVIEW.md`); Desktop apply RPC not implemented                        |
