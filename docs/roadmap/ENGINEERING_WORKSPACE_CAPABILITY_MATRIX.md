...
| Adoption plan / apply                         | implemented | adoption contracts                            | yes       | read-only `intentloom.existing-project.adoption.plan.v1`; apply still `APPROVED_APPLY` / CLI | `adopt`, `update`                        | typed client, no wizard UI | partial                 | partial                               | `integration-pending` | Preview reuses `adoptProject({ dryRun: true })`; Desktop apply not started                                                                                        |
