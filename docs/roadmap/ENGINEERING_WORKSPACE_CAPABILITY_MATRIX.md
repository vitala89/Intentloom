...
| Adoption plan / apply                         | implemented | adoption contracts                            | yes       | read-only plan/decisions/prepare/revalidate/approve RPCs; apply still `APPROVED_APPLY` / CLI | `adopt`, `update`                        | preview + decisions + prepared-plan + explicit approval | partial                 | partial                               | `integration-pending` | Explicit approval is read-only authorization evidence; Desktop apply not started                                                                                  |
