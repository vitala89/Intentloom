...
| I6 Transactional apply       | `applyFoundationProjectScaffold`, `rollbackFoundationProjectScaffold` (wrap I6)                                | `foundation-scaffold-apply.test.ts`    | foundation apply/rollback RPC | CLI `scaffold-apply`/`scaffold-rollback` | W7 Core complete on `main` (`1e54350`); Client in progress | Foundation gate + revalidation; Desktop/TUI in W7 Client                    |
...
| **W7**  | Transactional empty-root creation                                                          | Core merged on `main` (`1e54350`, PR #300); Client in progress      | `core-first` Core complete; Client PR follows Core  |
