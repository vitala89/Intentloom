...
1. **`ui`** — interactive TUI state rendering.
2. **`workspace`** — agent workspace conversation modes.
3. **`neutron`** — subagent + sync.
4. **`memory` / `session` / `security`** — large controlled-learning and security
   surfaces; extract as separate PRs (do not combine).
5. **`adopt` / `update` / `sync` / `diff` / `plan` / `init`** — project mutation
   and sync family; extract last because of shared transaction helpers.
6. **`summary` / `skill` / `proposal` / `evaluate` / `checkpoint` / `profile` /
