...
Candidate baseline under review: `b8f1e31` (`main` and `origin/main` after PR
#106 merge). The documentation updates in this branch still require review
...
| 4 — Security and supply chain     | [V1_SECURITY_AND_SUPPLY_CHAIN_AUDIT.md](../security/V1_SECURITY_AND_SUPPLY_CHAIN_AUDIT.md), `tests/v1-security-supply-chain.test.ts`, `.github/workflows/dependency-review.yml` | Control merged in `86a1aee`; high fast-uri alert closed; medium glib alert #2 assessed but not yet accepted or remediated |
...
2. Choose one disposition for Dependabot alert #2: plan and validate a
   coordinated GTK/WebKit/Tauri-compatible upgrade, or record an explicit
   maintainer exception with rationale, scope, and review date.
