...
Candidate baseline under review: `c49bf793` (`main` and `origin/main` after PR
#119 merge). The release gate remains open until the decisions and evidence in
...
for slower Windows runners, and PR #118 is merged as documentation-only; neither
...
runtime-equivalent candidate tree. The current post-merge run for `c49bf793`
has one Windows Node 24 packed-process timeout failure; a scoped test-only
remediation is under review. The v1.0 release gate remains **open** because support-policy
...
| Supported Node/host/provider matrix                          | BLOCKED             | [COMPATIBILITY_MATRIX.md](../compatibility/COMPATIBILITY_MATRIX.md); post-merge run [30459836027](https://github.com/vitala89/Intentloom/actions/runs/30459836027) for `c49bf793` failed only on Windows Node 24 at `tests/adapter-packed-process.test.ts:96`; scoped timeout remediation is under review |
...
- PR [#119](https://github.com/vitala89/Intentloom/pull/119) merged as
  ...
  five jobs passed; a scoped test-only timeout remediation is under review.
...
The newer `main` candidate `c49bf793` is blocked by the Windows Node 24 timeout
in [run 30459836027](https://github.com/vitala89/Intentloom/actions/runs/30459836027).
...
the exact approved release commit `c49bf793` or authorize a fresh run after the
timeout remediation is merged.
...
3. Local release-candidate install, build, packed CLI smoke, and full validation
   are recorded above, but the current merged candidate `c49bf793` is blocked by
   the Windows Node 24 timeout until the remediation is merged. Confirm that the retained clean-room installation and explicit-path
   evidence from runtime-equivalent tree `46a278c` is sufficient for the exact
   approved commit, or authorize a fresh run.
