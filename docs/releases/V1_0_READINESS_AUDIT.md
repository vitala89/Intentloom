...
Candidate baseline under review: `86a1aee` (`main` and `origin/main` after PR
#105 merge). The documentation updates in this branch still require review
...
| 4 — Security and supply chain     | [V1_SECURITY_AND_SUPPLY_CHAIN_AUDIT.md](../security/V1_SECURITY_AND_SUPPLY_CHAIN_AUDIT.md), `tests/v1-security-supply-chain.test.ts`, `.github/workflows/dependency-review.yml` | Control merged in `86a1aee`; high fast-uri alert closed; medium glib alert #2 and release provenance checks remain |
...
| Threat model, permissions, provenance, and incident response | PENDING             | PR #105 merged with green Dependency Review and closed high alert #1; medium glib alert #2 requires upgrade assessment or approved exception before release         |
...
- Dependabot alert [#2](https://github.com/vitala89/Intentloom/security/dependabot/2)
  ...
  patched version. The Desktop lockfile uses the GTK 0.18.x family, so the
  compatible upgrade path must be assessed before changing it.

## Required actions before approval

1. Review and approve [SUPPORT_POLICY_V1.md](SUPPORT_POLICY_V1.md).
2. Assess and either remediate Dependabot alert #2 through a compatible GTK/WebKit
   upgrade or record an explicit maintainer exception with rationale.
