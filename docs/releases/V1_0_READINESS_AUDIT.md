...
Candidate baseline under review: `d750acf` (`main` and `origin/main` after PR
#125 merge). The release gate remains open until the decisions and evidence in
...
second bounded test-harness timeout, and PR #121, PR #122, PR #123, PR #124, and PR #125 are
...
runtime-equivalent candidate tree. The post-merge run for `d750acf` passed all
...
| Supported Node/host/provider matrix                          | PASS                | [COMPATIBILITY_MATRIX.md](../compatibility/COMPATIBILITY_MATRIX.md); post-merge run [30489057541](https://github.com/vitala89/Intentloom/actions/runs/30489057541) for `d750acf` passed all six Ubuntu, macOS, and Windows Node 22/24 jobs              |
...
the current `main` candidate `d750acf` passed post-merge in [run
30489057541](https://github.com/vitala89/Intentloom/actions/runs/30489057541).
...
the exact approved release commit `d750acf` or authorize a fresh run.
...
3. Local release-candidate install, build, packed CLI smoke, and full validation
   ...
   approved commit `d750acf`, or authorize a fresh run.
